...
if (isset($_GET['id'])) {
    ...
    if (!$contact) {
        exit('Δεν υπαρχει επαφη με αυτό το ID!');
    }
} else {
...
<?=template_header('Επεξεργασία Επαφής')?>

<div class="content update">
	<h2>Ενημέρωση επαφής #<?=$contact['id']?></h2>
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <label for="title">Ιδιότητα</label>
        ...
        <a class="back-button" href="javascript:history.back(1)">Ακύρωση</a>
    </form>
    <?php if ($msg): ?>
    <p><?=$msg?></p>
    <?php endif; ?>
